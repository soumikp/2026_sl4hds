...
fig2b <- ggplot(data_mmc, aes(x = x1, y = x2, color = y)) +
  ...
  #geom_point(data = support_vectors_mmc, aes(x = x1, y = x2), color = 'purple', size = 5, shape = 1) +
  # Add the separating hyperplane
  geom_abline(intercept = -b - 0.36, slope = -w[1, 1], color = "red", linetype = "solid", size = 1) +
  # Add the margin lines
  geom_abline(intercept = (0.55 - b- 0.36), slope = -w[1, 1], color = "blue", linetype = "dashed") +
  geom_abline(intercept = (-0.55 - b- 0.36), slope = -w[1, 1] , color = "blue", linetype = "dashed") +
  ...
  theme_bw() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5), axis.title = element_text(size = 24))
